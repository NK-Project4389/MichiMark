...
MichiMarkの実装自動実行スケジュールを**ユーザー指定時刻（JST）**に登録するスキル。

ユーザーが「XX:XXに実行」「XX時にお願い」などと指定した時刻を使う。
時刻が指定されていない場合はユーザーに確認する。
...
を実行して `日 月 年`（UTC）を取得する。

---

### ステップ2: 指定時刻をUTCに変換してcron式を生成

ユーザーが指定した時刻はJST（UTC+9）なので、9時間引いてUTCに変換する。

例：
- 23:00 JST → 14:00 UTC → `0 14 DD MM *`
- 03:00 JST（翌日扱い） → 18:00 UTC（前日） → 日付を1日前に調整

**日付の扱い：**
- 変換後のUTC時刻が現在UTC時刻より過去になる場合は翌日の日付を使う
- 時が24以上になる場合は日を繰り上げる
- 時が0未満になる場合は日を1日前に戻す（月またぎは月も調整）
...
- **action**: `"create"`
- **name**: `"MichiMark-impl-[YYYYMMDD-HHMM(JST)]"`
