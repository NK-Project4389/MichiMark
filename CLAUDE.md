...
AIはセッション開始時に必ず `docs/Progress/README.md` を確認し、最新の進捗ファイルの「次回やること」セクションだけを読んでから作業を開始すること。

- 記録場所: `docs/Progress/YYYY-MM-DD_[作業内容].md`
- セッション終了時またはユーザーが記録を求めた際に進捗MDを作成・更新する
- 記録内容：完了サマリー（2〜3行）・未完了タスク・次回やること
...
セッション開始時は最新進捗ファイルの **「次回やること」セクションのみ** を読む。完了済み作業の詳細は読まない。

### 2. Roadmap.mdを現在地の参照点にする

`docs/Roadmap.md` の「現在の実装状況」セクションを常に最新に保つ。毎回進捗ファイルを辿らなくても現在地がわかるようにする。

### 3. 役割ごとに読むドキュメントを限定する

| 役割 | 読むべきファイル |
|---|---|
| `flutter-dev` | 対象FeatureのSpec + 最新進捗の「次回やること」のみ |
| `architect` | 設計憲章 + 対象Specテンプレート + 最新進捗の「次回やること」 |
| `product-manager` | `docs/Roadmap.md` + 対象Requirements |
| `reviewer` | 対象FeatureのSpec + レビュー対象ファイル |

役割に関係のないドキュメントは読まない。

### 4. Specを薄く保つ

- SpecはフィールドとEvent・Delegateの定義のみ記載する
- 実装の「なぜ」はコメントではなくコミットメッセージに書く
- Spec肥大化を防ぎ、読み込みコストを抑える
