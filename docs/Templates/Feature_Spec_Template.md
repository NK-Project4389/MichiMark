...
architectがSpecを作成する際に、ユーザー目線のテストシナリオを定義する。
testerはこのシナリオのみを参照してIntegration Testを実装する（ブラックボックス）。

## 前提条件

テスト実行前に準備が必要な状態を記述する。

例:
- iOSシミュレーターが起動済みであること
- テスト用データが存在すること（または存在しないこと）

## テストシナリオ一覧

| ID | シナリオ名 | 優先度 |
|---|---|---|
| TC-001 | （例）地点の新規登録 | High |
| TC-002 | （例）地点の編集 | High |
| TC-003 | （例）地点の削除 | High |
| TC-004 | （例）画面遷移（一覧→詳細→一覧） | Medium |
