...
| MichiInfo日付区切り表示 | TC-DS | 7 | タイムラインへの日付区切りウィジェット挿入・InsertMode時非表示・Canvas描画連続性（UI-23） |
...
| TC-DS-004 | 区切りの日付テキストが `yyyy/MM/dd` 形式で表示される | High |
| TC-DS-005 | InsertMode中に区切りが非表示になる | Medium |
| TC-DS-006 | Markカード・Linkカードの縦幅が変化しない | Medium |
...
**前提:** イベント詳細画面でvisitWorkトピックのMarkが同一日付で1件存在する。

**操作手順:**
1. EventListからイベントを開く
2. ミチタブを表示する
3. タイムラインが描画されるまで待つ

**期待結果:**
- Markカードの直上に日付区切りが1件表示される
- `Key('michiInfo_dateSeparator_0')` が見つかる
...
**前提:** イベント詳細画面でvisitWorkトピックのMarkが同一日付で2件以上存在する。
...
- 2件目以降のMarkカードの直上に区切りは表示されない
- `Key('michiInfo_dateSeparator_0')` が見つかる
- `Key('michiInfo_dateSeparator_1')` は見つからない

---

### TC-DS-003: 日付が変わるMarkの直前に区切りが挿入される（2日分のMarkがある場合）

**前提:** イベント詳細画面でvisitWorkトピックのMarkが異なる2日付に存在する（日付A < 日付B）。
...
- 日付Aのカード直上に区切りが表示される
- 日付Bのカード直上に区切りが表示される
- `Key('michiInfo_dateSeparator_0')` と `Key('michiInfo_dateSeparator_1')` が見つかる

---

### TC-DS-004: 区切りの日付テキストが `yyyy/MM/dd` 形式で表示される

**前提:** Mark日付が `2024/08/11` で、イベント詳細のミチタブが表示されている。
...
**期待結果:**
- 区切りウィジェット内に `2024/08/11` というテキストが表示される
- `find.text('2024/08/11')` が区切りウィジェット内で見つかる

---

### TC-DS-005: InsertMode中に区切りが非表示になる

**前提:** イベント詳細のミチタブが表示されている。Markが1件以上存在する。

**操作手順:**
1. EventListからイベントを開く
2. ミチタブを表示する
3. FABボタン（`Key('michiInfo_button_insertModeFab')`）をタップしてInsertModeを有効にする
...
**期待結果:**
- Markカード・Linkカードが表示される
- 区切りウィジェットが表示される
- Mark/Linkカードのタップ操作（詳細画面遷移）が正常に動作する

---

### TC-DS-007: 道路帯Canvas（グレー帯・白破線）が区切り位置で切断されない

**前提:** イベント詳細のミチタブが表示されている。異なる日付のMarkが2件以上存在する。
...
- 道路帯（グレー帯・白破線）が区切りウィジェットを貫通して連続して描画される
- CustomPaintウィジェットが存在し、タイムラインの縦線が途切れない
