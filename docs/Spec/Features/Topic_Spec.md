...
| `availableTopics` | `List<TopicDomain>` | 選択可能なTopic一覧（EventDetailBlocから渡される） |
...
| `BasicInfoAvailableTopicsReceived` | EventDetailBlocからTopic一覧が渡されたとき | Draft.availableTopicsを更新する |
...
| `BasicInfoTopicChangedDelegate` | EventDetailBloc | Topic変更をEventDetailBlocに通知する（BlocListenerがEventDetailBlocへEventを送信する） |

> [設計判断] `BasicInfoTopicChangedDelegate` の通知経路: BasicInfoBlocがDelegateをStateに乗せ、EventDetailPageのBlocListenerがそれを受け取って `EventDetailTopicChanged` をEventDetailBlocに送信する。これにより子Bloc→親BlocへのEvent直接送信を避け、Widget（BlocListener）を仲介することでレイヤーの一方向依存を維持する。DelegateはUI遷移ではなく「選択結果の通知」として設計憲章§8で許可されている用途に該当する。
...
- `BasicInfoTopicSelected` 処理時に Draft.selectedTopic を更新し、TopicConfigを再生成してStateに反映する。同時に `BasicInfoTopicChangedDelegate` をStateに乗せる
- `BasicInfoAvailableTopicsReceived` 処理時に Draft.availableTopics を更新する
- `TopicRepository` はコンストラクタ注入しない。availableTopicsはEventDetailBlocから受け取る（§11参照）
...
- `domain.topic` の初期値をもとにTopicRepositoryから availableTopics を取得し、`BasicInfoAvailableTopicsReceived` をBasicInfoBlocに送信する
- `EventDetailTopicChanged` 受信時にTopicConfigを再生成し、`MarkDetailTopicConfigUpdated` / `LinkDetailTopicConfigUpdated` / `MichiInfoTopicConfigUpdated` を各子Blocへ送信する
- EventDetailBlocはTopicRepositoryをDI（get_it）経由でコンストラクタ注入する

## TopicRepository DI責務の設計判断

> BasicInfoBlocはTopicRepositoryをDI注入しない。availableTopicsの取得責務はEventDetailBlocに集約する。
> 理由: EventDetailBlocはEventDomainを所有しTopicConfigを管理する責務を既に持つ。Topicリスト取得もEventDetailに集約することで責務の一貫性を維持し、BasicInfoBlocの責務範囲をドラフト編集に限定できる。
> EventDetailBlocは起動時にTopicRepository.fetchAll()を呼び出し、結果を `BasicInfoAvailableTopicsReceived` イベント経由でBasicInfoBlocに渡す。

## EventDetailEvent 追加

| Event名 | 発火タイミング | 説明 |
|---|---|---|
| `EventDetailTopicChanged` | EventDetailPageのBlocListenerがBasicInfoTopicChangedDelegateを受け取ったとき | TopicConfigを再生成し子Blocへ伝播する |
...
- BasicInfoBloc が Draft.selectedTopic を更新し、TopicConfigを再生成してStateに反映する。同時に `BasicInfoTopicChangedDelegate` をStateに乗せる
- EventDetailPage の BlocListener が `BasicInfoTopicChangedDelegate` を受け取り、`EventDetailTopicChanged` を EventDetailBloc に送る
- EventDetailBloc が TopicConfig を再生成し、各子Bloc（MichiInfoBloc・MarkDetailBloc・LinkDetailBloc）へ対応するTopicConfigUpdatedイベントを一方向で送る
- 各子Blocは受け取ったイベントで自身のStateを自律的に更新する（子BlocはEventDetailBlocのStateを参照しない）
...
| `BasicInfoAvailableTopicsReceived` | EventDetailBlocから初期化時に渡されたとき | `List<TopicDomain> topics` |

## EventDetailBloc 追加Event

| Event名 | 発火タイミング | ペイロード |
|---|---|---|
| `EventDetailTopicChanged` | EventDetailPageのBlocListenerがBasicInfoTopicChangedDelegateを受け取ったとき | `TopicDomain? topic` |
...
| Delegate名 | 所属Feature | 受取先 | 遷移・処理内容 |
|---|---|---|---|
| `BasicInfoOpenTopicSelectionDelegate` | BasicInfoBloc | BasicInfoViewのBlocListener | `/selection`（eventTopic, single）へ `context.push` する |
| `BasicInfoTopicChangedDelegate` | BasicInfoBloc | EventDetailPageのBlocListener | `EventDetailTopicChanged` をEventDetailBlocへ送信する |

> [設計判断] `BasicInfoTopicChangedDelegate` をDelegateとして使用する根拠: Delegateは「選択結果の通知」として設計憲章§8で許可された用途に該当する。BlocからBlocへEventを直接送信することはレイヤー境界を侵害するため、Widget（BlocListener）を仲介する形でEventDetailBlocへの通知を実現する。これにより子Bloc→親Blocへの直接依存を排除し、一方向依存を維持する。
