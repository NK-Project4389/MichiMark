...
> `/event/link/{uuid}` へ遷移する。LinkDetailBloc は UUID を生成しない。
>
> **ルート統合:** `/event/link/new` ルートは廃止。新規・編集ともに `/event/link/:linkId` に統合。
