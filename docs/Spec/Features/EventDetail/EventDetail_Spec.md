...
EventDetailStarted(eventId) を受けたとき、Repository に該当 Event が存在しない場合（NotFoundError）は
新規作成モードとして扱い、以下の空ドメインを生成して Repository に保存してから画面をロードする。

```
EventDomain(
  id: eventId,        // router から渡された UUID
  eventName: '',
...
> **UUID生成タイミング:** EventList の BlocListener で `Uuid().v4()` を生成し、
> `/event/{uuid}` へ遷移する。EventDetailBloc は UUID を生成しない。
